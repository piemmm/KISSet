 ...
 *     distribution.
 *
 * $Id: CompressedBlockInputStream.java,v 1.1 2015/06/16 20:23:42 ihawkins Exp $
 */
...
public class CompressedBlockInputStream extends FilterInputStream {
   private static final Log LOG = LogFactory.getLog("CompressedBlockInputStream");
   ...
   private void readAndDecompress() throws IOException {
      ...
      if (outLength == 0) {
         outBuf = new byte[inLength];
         // Uncompressed
         for (int i = 0; i < inLength; i++) {
            outBuf[i] = (byte) in.read();
         }
         outLength = inLength;

//         System.out.println("Read:"+new String(outBuf, 0, inLength));
         ...
         try {
            int read = inflater.inflate(outBuf);
            if (read == 0 && inflater.needsDictionary()) {
               // System.out.println("Dict set");
               // try { Thread.sleep(500); } catch(Throwable e) {}
               inflater.setDictionary(dict.getDictionary());
      ...
      LOG.debug("Decompressed " + inLength + " bytes into " + outLength + " bytes");
      dict.addToDictionary(outBuf, 0, outLength);
   }
