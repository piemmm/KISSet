...
public class UnHeard extends Command {
    ...
    public boolean doCommand(String[] data) throws IOException {
        ...
        if (nodes.size() == 0) {
            ...
            write(ANSI.UNDERLINE + ANSI.BOLD + "Int  Callsign  Last UnHeard       CanReach" + ANSI.NORMAL + CR);
            ...
            for (Node node : nodes) {
                ...
                write(StringUtils.rightPad(Integer.toString(connectors.indexOf(node.getInterface())), 5) + StringUtils.rightPad(node.getCallsign(), 10) + StringUtils.rightPad(sdf.format(node.getLastHeard()), 18) + " " + StringUtils.rightPad(canReach(node), 14) + CR);
            }
        }
        return true;
    }
