 ...
 */
@InterfaceDriver(name = "KISS via traditional TNC", description = "KISS via TNC enabling KISS", uiName = "fx/TNCConnectionPreference.fxml")
public class KISSviaTNC extends Interface {

    private static final Log LOG = LogFactory.getLog("KISSviaTNC");
    ...
    public KISSviaTNC(HierarchicalConfiguration config) {
        super(config);
    ...
    public void setup() {
        ...
        if (in == null || out == null) {
            LOG.error("Unable to connect to TNC device at: " + port + " - this connector is stopping.");
            running = false;
            return;
        }

        // Put the TNC into KISS mode.
        try {
            out.write("KISS ON\n".getBytes());
            out.flush();
            out.write("RESTART\n".getBytes());
            out.flush();
            out.write("INTERFACE KISS\n".getBytes());
            out.flush();
            out.write("RESET\n".getBytes());
            out.flush();
            Tools.delay(2000);
        ...
        anInterface.addFrameListener(new AX25FrameListener() {
            ...
            public void consumeAX25Frame(AX25Frame frame, Connector connector) {
                ...
                Node node = new Node(KISSviaTNC.this, frame.sender.toString(), frame.rcptTime, frame.dest.toString(), frame);
    ...
    @Override
    public void stop() {
        // Drop the TNC out of kiss mode.
        try {
            synchronized(out) {
                // Send the end kiss bytes
                out.write(0xC0);
                out.write(0xFF); // End KISS command
                out.write(0xC0);
                out.flush();
                Tools.delay(100);
                out.write("KISS OFF\n".getBytes());
